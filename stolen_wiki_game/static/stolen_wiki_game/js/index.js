...
function LoadSave() {
    ...
    if (localStorage.getItem("redactleSavet") === null) {
        localStorage.clear();
        playerID = uuidv4();
        save = JSON.parse(JSON.stringify({
            "saveData": {
                redactleIndex,
                guessedWords,
                gameWins,
                ...
                gameAccuracy,
                gameAnswers
            }, "prefs": {hidingZero, hidingLog, pluralizing}, "id": {playerID}
        }));
    } else {
        save = JSON.parse(localStorage.getItem("redactleSavet"));
        // wipe previous guesses
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                redactleIndex = save.saveData.redactleIndex || 1;
...
function SaveProgress(incrementIndex = false) {
    ...
    // increment on victories to keep track of player progress
    if(incrementIndex) {
        save.saveData.redactleIndex++;
    }
